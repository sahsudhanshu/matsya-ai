...
const ENV_CHECKS = [
    ...
    ['AWS_REGION', 'AWS_REGION', 'warn', 'AWS region for S3/Polly'],
    ...
    ['DB_HOST', 'DB_HOST', 'critical', 'MySQL database host'],
    ['DB_USER', 'DB_USER', 'critical', 'MySQL database user'],
...
async function runStartupChecks() {
    ...
    try {
        ...
        console.log(`${c.cyan}║${c.reset}  ${OK} ${'MySQL'.padEnd(26)} = ${c.green}connected${c.reset} ${c.dim}(${count} tables)${c.reset}`);
    } catch (err) {
        console.log(`${c.cyan}║${c.reset}  ${FAIL} ${'MySQL'.padEnd(26)} = ${c.red}unreachable${c.reset}  ${c.dim}${err.message?.slice(0, 50)}${c.reset}`);
        results.criticalErrors++;
    }
