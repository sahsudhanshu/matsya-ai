...
// array ternary
code = code.replace(/style=\{\s*\[\s*styles\.([a-zA-Z0-9_]+)\s*,\s*([\s\S]*?)\s*\?\s*styles\.([a-zA-Z0-9_]+)\s*:\s*styles\.([a-zA-Z0-9_]+)\s*,?\s*\]\s*\}/g, (m, g1, g2, g3, g4) => {
  return `className={\`${classMap[g1]} \${${g2.trim()} ? '${classMap[g3]}' : '${classMap[g4]}'}\`}`;
});

// array AND
code = code.replace(/style=\{\s*\[\s*styles\.([a-zA-Z0-9_]+)\s*,\s*([\s\S]*?)\s*&&\s*styles\.([a-zA-Z0-9_]+)\s*,?\s*\]\s*\}/g, (m, g1, g2, g3) => {
  return `className={\`${classMap[g1]} \${${g2.trim()} ? '${classMap[g3]}' : ''}\`}`;
});

// array 2 styles simple
code = code.replace(/style=\{\s*\[\s*styles\.([a-zA-Z0-9_]+)\s*,\s*styles\.([a-zA-Z0-9_]+)\s*,?\s*\]\s*\}/g, (m, g1, g2) => {
  return `className="${classMap[g1]} ${classMap[g2]}"`;
});

// array with inline object
code = code.replace(/style=\{\s*\[\s*styles\.([a-zA-Z0-9_]+)\s*,\s*(\{[\s\S]*?\})\s*,?\s*\]\s*\}/g, (m, g1, g2) => {
  return `className="${classMap[g1]}" style={${g2}}`;
});

// single
code = code.replace(/style=\{\s*styles\.([a-zA-Z0-9_]+)\s*\}/g, (m, g1) => {
  return `className="${classMap[g1]}"`;
});

const remaining = code.match(/style=\{.*?\}/g);
if (remaining) {
  console.log("Remaining styles:");
  remaining.forEach(r => console.log(r));
} else {
  console.log("All matched styles replaced.");
}
